...
export default async function Niebla() {
  ...
  return <div className="niebla">
    ...
    <section id="historias" className="n-chapter n-future">
      ...
      <article className="n-feature"><Link href={`/niebla/${lead.slug}`} className="n-feature-image" aria-label={`Ver propuesta: ${lead.title}`}><img src={lead.image} alt={lead.imageAlt} width={1280} height={720} loading="lazy" /><span>Propuesta de entrevista</span></Link><div className="n-feature-text"><p className="n-label">Protagonistas / Número cero</p><h3><Link href={`/niebla/${lead.slug}`}>{lead.title}<br /><em>{lead.subtitle}</em></Link></h3><p>Una conversación por preparar sobre empleo, oportunidades y el futuro del territorio.</p><p className="n-demo-note">{lead.notice}</p><Link className="n-story-link" href={`/niebla/${lead.slug}`}>Explorar la entrevista <span>↗</span></Link><small>{lead.credit}</small></div></article>
    </section>
